...
class ImageEnhancerV2(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(ImageEnhancerV2, self).__init__(opt)
    ...
    def feed_train_data(self, data):
        ...
        self.conds = downsample_fft(cal_z_gt(self.lq, self.gt, self.alpha), sigma=1)
        self.conds = self.conds + torch.randn_like(self.conds) * noise_level
        self.conds = F.interpolate(self.conds, scale_factor=8, mode='bilinear', align_corners=False)
    ...
    def feed_data(self, data):
        ...
        self.conds = downsample_fft(cal_z_gt(self.lq, self.gt, self.alpha), sigma=1)
        self.conds = self.conds + torch.randn_like(self.conds) * noise_level
        self.conds = F.interpolate(self.conds, size=(self.lq.shape[-2], self.lq.shape[-1]), mode='bilinear', align_corners=False)
        self.conds = self.conds.to(self.device)
    ...
    def optimize_parameters(self, current_iter):
        ...
        with autocast(enabled=self.use_amp):
            if current_iter > self.opt['train']['scheduler']['periods'][0]:
                self.mask = None
            input_ = torch.cat([self.lq, self.conds], dim=1)
    ...
    def pad_test(self, window_size):
        ...
        cond_img = F.pad(self.conds, (0, mod_pad_w, 0, mod_pad_h), 'reflect')
        fea_map = self.nonpad_test(img, cond_img)
        _, _, h, w = self.output.size()
        self.output = self.output[:, :, 0:h - mod_pad_h * scale, 0:w - mod_pad_w * scale]
        return fea_map

    def nonpad_test(self, img=None, cond_img=None):
        if img is None:
            img = self.lq
        if cond_img is None:
            cond_img = self.conds
        input_ = torch.cat([img, cond_img], dim=1)
